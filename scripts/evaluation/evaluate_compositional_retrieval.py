"""Stage 6b — Evaluate additive concept retrieval at increasing query lengths.

Each held-out image is represented by standardized projections onto the
model's concept directions. A fixed-dictionary query containing concepts S
scores gallery item j as

    score(j | S) = sum(profile[j, k] for k in S).

For every CC0 image, query concepts are read from a JSONL field (normally the
``present`` field of exhaustive fixed-dictionary labels) and matched to the
selected dictionary. Fixed-seed random permutations provide nested subsets, so
retrieval can be measured as the concept budget grows. The same subsets are
used for every profile space.

The preferred input is the matched NPZ from ``build_retrieval_profiles.py``.
Static playground HTML remains supported for exact historical evaluation.
"""
...
def load_query_labels(path: str, field: str = "present") -> list[dict]:
    rows = []
    seen = set()
    with open(path) as f:
        for line_no, line in enumerate(f, 1):
            ...
            if not isinstance(values, list) or not all(
                ...
                raise ValueError(
                    f"{field} must be a nonempty string list at line {line_no}"
                )
            row = {**row, "query_concepts": values}
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="present",
        help=(
            "JSONL field containing query concepts; use 'present' for exhaustive "
            "fixed-dictionary label artifacts"
        ),
    )
    ap.add_argument("--split-manifest", default="data/splits.json")
    ap.add_argument("--eval-split", choices=("train", "dev", "test"), default="dev")
    ...
    fixed_labels_path = os.path.join(ROOT, args.fixed_labels)
    dictionary = load_dictionary(args.dictionary, args.dictionary_git)
    manifest = load_split_manifest(ROOT, args.split_manifest)
    rows = [
        row
        for row in load_query_labels(fixed_labels_path, args.label_field)
    ...
    if args.profiles_npz:
        ...
        for label, data in (("trained", trained), ("frozen", frozen)):
            ...
            if mismatches:
                raise ValueError(
                    f"{label} gallery order differs from fixed labels at rows "
                    f"{mismatches[:5]}"
                )
