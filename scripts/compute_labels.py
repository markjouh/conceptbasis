...
Backbone: ViT-B-16-SigLIP2-256. For every image:
  - 768-d image embedding (saved; doubles as the adapter's training features)
  - score against each of the 256 dictionary concept directions
    (mean member text emb - 'an object' base), GMM-calibrated to [0,1]

Outputs:
  data/image_embeddings.npy      [N, 768] float32 (L2-normalized)
  data/image_ids.json         image_id order for the matrix
  data/labels.parquet        image_id, concept folder, split, s_<concept>...
  data/concept_directions_initial.npy  [256, 768] prompt-based text directions
"""
